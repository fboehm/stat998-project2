...
tune1 = tune(svm, train.x=as.matrix(x.tr), train.y=as.factor(y.tr), kernel='linear', range=list(cost=2^seq(-5,5,length=100)), 
...
tune2 = tune(svm, train.x=as.matrix(x.tr), train.y=as.factor(y.tr), kernel='radial', range=list(cost=2^seq(-5:5), gamma=2^(-5:5)), control=tune.control(sampling='cross', cross=10))
...
tune.rf = tuneRF(x=x.tr, y=as.factor(y.tr), ntree=1000, mtryStart=10, stepFactor=1, nodesize=10)

## Fit the model
fit.rf  = randomForest(x=x.tr, y=as.factor(y.tr), ntree=1000, mtry=10, nodesize=10, importance=T)
